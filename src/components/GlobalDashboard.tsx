...
const mockActions: StrategyAction[] = [
  { id: '1', timestamp: new Date(), action: 'Engine rebalanced 50 BNB from AsterDEX to PancakeSwap Farm', type: 'rebalance' },
  { id: '2', timestamp: new Date(), action: 'Auto-harvested 125 CAKE rewards from LP Position', type: 'harvest' },
  { id: '3', timestamp: new Date(), action: 'Compounded $2,450 USDT yield into BNB-USDT LP', type: 'compound' },
  { id: '4', timestamp: new Date(), action: 'Hedging protocol activated: 15% moved to stablecoin vault', type: 'hedge' },
  { id: '5', timestamp: new Date(), action: 'Smart Router detected 0.3% APY gain on Riquid Pool', type: 'rebalance' },
];
...
export function GlobalDashboard() {
  ...
  const [actions] = useState<StrategyAction[]>(mockActions);
        ...
        <div className={`relative overflow-hidden rounded-2xl border p-6 ${cardClass}`}>
          ...
          <div className="relative">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-emerald-500 text-sm">All systems operational</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className={`rounded-2xl border overflow-hidden ${cardClass}`}>
        <div className={`px-6 py-4 border-b flex items-center justify-between ${
          ...
          <div className="flex items-center gap-2">
            ...
            <h3 className={`font-semibold ${theme === 'dark' ? 'text-white' : 'text-slate-900'}`}>Live Strategy Feed</h3>
          </div>
          <span className={`text-xs font-mono ${theme === 'dark' ? 'text-slate-500' : 'text-slate-400'}`}>REAL-TIME TERMINAL</span>
        </div>
              ...
              >
                ...
                <div className="flex-1">
                  ...
                  <p className={`text-xs mt-1 ${theme === 'dark' ? 'text-slate-600' : 'text-slate-400'}`}>
                    {new Date().toLocaleTimeString()} • Block #38,472,{Math.floor(Math.random() * 999)}
                  </p>
                </div>
                {index === currentActionIndex && (
